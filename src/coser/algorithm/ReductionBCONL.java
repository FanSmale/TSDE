...
public class ReductionBCONL {
	 ...
	 *************************** 
	 */
	public static String batchMinimalTimeCostReductionBCONL(int paraBeesCounts,
			int paraNeighborsUpperBound, int paraNeighborsLowerBound,
			int paraNeighborsStepLength, int paraExperiments) throws Exception {
		...
		try {
			...
			int numberOfNeighbors = 1 + (int) ((paraNeighborsUpperBound - paraNeighborsLowerBound) / paraNeighborsStepLength);
			if (numberOfNeighbors < 1) {
				message = "Neighbors lower bound: " + paraNeighborsLowerBound
						+ " is greater than the upper bound"
						+ paraNeighborsUpperBound;
				throw new Exception(
						"Error occurred in ReductionBCONL.batchMinimalTimeCostReductionBCONL(): \r\n"
								+ message);
			}// Of if
			int[] neighborsArray = new int[numberOfNeighbors];
			neighborsArray[0] = paraNeighborsLowerBound;
			for (int i = 1; i < numberOfNeighbors; i++) {
				neighborsArray[i] = neighborsArray[i - 1]
						+ paraNeighborsStepLength;
			}// Of for i
			...
			findingOptimalCount = new int[numberOfNeighbors];
			double exceedingFactor = 0;
			maximalExceedingFactor = new double[numberOfNeighbors];
			exceedingFactorSum = new double[numberOfNeighbors];
			...
			for (int i = 0; i < paraExperiments; i++) {
				...
				for (int j = 0; j < numberOfNeighbors; j++) {
					...
					currentSystem.setlimitsValue(neighborsArray[j]);
					currentSystem.setremoveRateValue(0.4);
					currentSystem.setGenerationsValue(15);

					// Let's have a look.
					currentSystem.beeColonyTimeReduction(paraBeesCounts, 15,
							neighborsArray[j], 0.4);
			...
			message += "\r\nResults/neighbor counts\tFinding optimal factor\t"
					+ "Maximal exceeding factor\tAverage exceeding factor\r\n";
			for (int i = 0; i < numberOfNeighbors; i++) {
				message += "" + neighborsArray[i];
