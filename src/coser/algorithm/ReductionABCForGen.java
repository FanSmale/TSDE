 ...
 */
public class ReductionABCForGen {
	...
	public static String batchMinimalTimeCostReductionABC(
			...
			int paraIterUpperBound, int paraIterLowerBound,
			int paraIterStepLength, double paraCPBest, double paraCGBest,
			int paraLimit, int paraExperiments) throws Exception {
		...
		try {
			...
			AbcFormalTimeNominalDecisionSystem currentSystem = new AbcFormalTimeNominalDecisionSystem(
					...
					paraNumberOfPopulation, paraFoodNumber, paraLimit, 50,
			...
			int numberOfIter = 1 + (int) ((paraIterUpperBound - paraIterLowerBound) / paraIterStepLength);
			if (numberOfIter < 1) {
				message = "Iteration lower bound: " + paraIterLowerBound
						+ " is greater than the upper bound"
						+ paraIterUpperBound;
				throw new Exception(
						"Error occurred in ReductionABCForGen.batchMinimalTimeCostReductionABC(): \r\n"
								+ message);
			}// Of if
			int[] iterArray = new int[numberOfIter];
			iterArray[0] = paraIterLowerBound;
			for (int i = 1; i < numberOfIter; i++) {
				iterArray[i] = iterArray[i - 1] + paraIterStepLength;
			}// Of for i
			...
			findingOptimalCount = new int[numberOfIter];
			double exceedingFactor = 0;
			maximalExceedingFactor = new double[numberOfIter];
			exceedingFactorSum = new double[numberOfIter];
			...
			for (int i = 0; i < paraExperiments; i++) {
				...
				for (int j = 0; j < numberOfIter; j++) {
					System.out.println("iterArray[ " + (j + 1) + " ]="
							+ iterArray[j]);
					...
					currentSystem.AbcTestTimeReduction(paraNumberOfPopulation,
							paraFoodNumber, paraLimit, iterArray[j],
							paraCPBest, paraCGBest);
			...
			message += "\r\nResults/iteration\tFinding optimal factor\t"
