...
public class ReductionFRWithRestart {
	 ...
	 *************************** 
	 */
	public static String batchMinimalTestCostReductionFRWithRestart(
			int paraAlpha, int paraBeta, double paraDelta, int paraseed,
			int paraRestartTime, int paraExperiments) throws Exception {
		String message = "";
		Date date1 = new Date();
		...
		try {
			...
			TestCostNominalDecisionSystemFastRandom currentSystem = new TestCostNominalDecisionSystemFastRandom(
					...
					paraDelta, paraseed, paraRestartTime);
			System.out.print("Fast randomized algorithm with re-start \n");
			System.out
					.println("batchMinimalTestCostReductionFRWithRestart: 测试代价数组："
			...
			for (int i = 0; i < paraExperiments; i++) {
				...
				for (int j = 0; j < numberOfBeta; j++) {
					...
					double tempTestCost = currentSystem
							.minimalTestCostReductFRWithRestart();
