 ...
 */
public class GraftSplit extends ClassifierSplitModel implements
		Comparable<Object> {
	 ...
	 * @param v
	 *            the value of a where split occurs
	 * @param t
	 *            the test type (0 is <=, 1 is >, 2 is =, 3 is !)
	 * @param c
	 *            the class to label the leaf node pointed to by test as.
	 * @param l
	 ...
	 *            the test type (0 is <=, 1 is >, 2 is =, 3 is !=)
	 * @param oC
	 *            the class to label the leaf node not pointed to by test as.
	 * @param counts
	 *            the distribution for this split
	 */
	...
	/**
	 * deletes the cases in data that belong to leaf pointed to by the test
	 * (i.e. the subset of interest). this is useful so the instances belonging
	 * to that leaf aren't passed down the other branch.
	 * 
	 * @param data
	 ...
	 * 
	 * @param data
	 *            the instances to use when creating the distribution
	 */
	public void buildClassifier(Instances data) throws Exception {

		// distribution for the graft, not counting cases in atbop, only orig
		// leaf
	...
	/**
	 * @return the NoSplit object for the leaf not pointed to by m_testType
	 *         branch
	 */
	 ...
	 * 
	 * @param index
	 *            the bag to dump label for
	 * @param data
	 *            to get attribute names and such
	...
	public final String dumpLabelG(int index, Instances data) throws Exception {
		...
		text.append(((Instances) data).classAttribute().value(
				(index == subsetOfInterest()) ? m_maxClass
						: m_otherLeafMaxClass));
		...
		if (index == subsetOfInterest()) {
			text.append("|"
					+ Utils.roundDouble(m_distribution.perBag(index), 2));
			if (Utils.gr(m_distribution.numIncorrect(index), 0))
				text.append("/"
						+ Utils.roundDouble(m_distribution.numIncorrect(index),
								2));
		}
		text.append(")");
	...
	/**
	 * @param subset
	 *            the subset to get the positives for
	...
	/**
	 * @param subset
	 *            the index of the bag to get the total for
	...
	public final String sourceExpression(int index, Instances data) {
		...
		if (data.attribute(m_attIndex).isNominal()) {
			...
			expr.append(".equals(\"")
					.append(data.attribute(m_attIndex)
	...
	/**
	 * @param instance
	 *            the instance to produce the weights for
	...
	/**
	 * @param instance
	 *            the instance for which to determine the subset
	 ...
	 * 
	 * @param g
	 *            the graft split to compare to this one
	 * @return -1, 0, or 1 if this GraftSplit laplace is <, = or > than that of
	 *         g
	 */
	 ...
	 * @param instance
	 *            the instance to get the probability for
	 * @param theSubset
	 *            the subset
	 */
	 ...
	 * 
	 * @param data
	 *            instances for determining names of attributes and values
