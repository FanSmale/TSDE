/*
 *  Copied from PruneableClassifierTree.java
 *
 */
...
public class PruneableClassifierTree extends ClassifierTree {
	...
	public PruneableClassifierTree(ModelSelection toSelectLocModel,
			boolean pruneTree, int num, boolean cleanup, int seed)
			throws Exception {
	...
	protected ClassifierTree getNewTree(Instances train, Instances test)
		...
		PruneableClassifierTree newTree = new PruneableClassifierTree(
				m_toSelectModel, pruneTheTree, numSets, m_cleanup, m_seed);
			...
			for (int i = 0; i < m_sons.length; i++)
				if (Utils.eq(localModel().distribution().perBag(i), 0)) {
					errors += m_test.perBag(i)
							- m_test.perClassPerBag(i, localModel()
									.distribution().maxClass());
				} else
