...
public class NBTreeSplit extends ClassifierSplitModel {
	...
	private void handleEnumeratedAttribute(Instances trainInstances)
		 ...
		 * double [m_complexityIndex]; for (int i = 0; i < m_complexityIndex;
		 * i++) { m_weights[i] = trainingSets[i].sumOfWeights(); }
		...
		for (int i = 0; i < m_complexityIndex; i++) {
			if (trainingSets[i].numInstances() >= 5) {
				...
				m_errors += NBTreeNoSplit.crossValidate(fullModel,
						trainingSets[i], r);
			} else {
		...
		// Check if there are at least five instances in at least two of the
		// subsets
		// subsets.
		if (minNumCount > 1) {
	 ...
	 * 
	 * @param global
	 *            a <code>NBTreeNoSplit</code> value
	 */
