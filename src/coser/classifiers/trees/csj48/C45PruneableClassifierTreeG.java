/*
 *    Copied from C45PruneableClassifierTreeG.java
 *
 */
...
public class C45PruneableClassifierTreeG extends ClassifierTree {
	...
	public C45PruneableClassifierTreeG(ModelSelection toSelectLocModel,
			boolean pruneTree, float cf, boolean raiseTree, boolean relabel,
	...
	/**
	 * Constructor for pruneable tree structure. Used to create new nodes in the
	 * tree during grafting.
	 * 
	 * @param toSelectLocModel
	 *            selection method for local splitting model
	 * @param data
	 *            the data used to produce split model
	 * @param gs
	 *            the split model
	 * @param prune
	 *            true if the tree is to be pruned
	 ...
	 */
	public C45PruneableClassifierTreeG(ModelSelection toSelectLocModel,
			Instances data, ClassifierSplitModel gs, boolean prune, float cf,
			boolean raise, boolean isLeaf, boolean relabel, boolean cleanup) {
	...
	protected ClassifierTree getNewTree(Instances data) throws Exception {
		...
		C45PruneableClassifierTreeG newTree = new C45PruneableClassifierTreeG(
	...
	private double getEstimatedErrors() {
		...
		if (m_isLeaf)
			return getEstimatedErrorsForDistribution(localModel()
					.distribution());
		else {
			for (i = 0; i < m_sons.length; i++)
	 ...
	 * @return the estimated errors
	 */
	private double getEstimatedErrorsForDistribution(
	...
	/**
	 * Initializes variables for grafting. sets up limits array (for numeric
	 * attributes) and calls the recursive function traverseTree.
	 * 
	 * @param data
	...
	/**
	 * recursive function. if this node is a leaf then calls findGraft,
	 * otherwise sorts the two sets of instances (tracked in iindex array) and
	 * calls sortInstances for each of the child nodes (which then calls this
	 * method).
	 * 
	 * @param fulldata
	 *            all instances
	 * @param iindex
	 *            array the tracks the weight of each instance in the atbop and
	 *            at the leaf (0.0 if not present)
	 * @param limits
	 *            array specifying current upper/lower limits for numeric atts
	 * @param parent
	 *            the node immediately before the current one
	 * @param pL
	 *            laplace for node, as calculated by parent (in case leaf is
	 *            empty)
	 * @param nodeClass
	 *            class of node, determined by parent (in case leaf empty)
	 */
	private void traverseTree(Instances fulldata, double[][] iindex,
			double[][] limits, C45PruneableClassifierTreeG parent, double pL,
			int nodeClass) throws Exception {

		if (m_isLeaf) {

			findGraft(fulldata, iindex, limits, (ClassifierTree) parent, pL,
					nodeClass);
			...
			for (int i = 0; i < localModel().numSubsets(); i++) {
				...
				for (int x = 0; x < 2; x++)
					System.arraycopy(iindex[x], 0, newiindex[x], 0,
							iindex[x].length);
	...
	/**
	 * sorts/deletes instances into/from node and atbop according to the test
	 * for subset, then calls traverseTree for subset's node.
	 * 
	 * @param fulldata
	 *            all instances
	 * @param iindex
	 ...
	 *            array specifying current upper/lower limits for numeric atts
	 * @param subset
	 *            the subset for which to sort instances into inode & iatbop
	 */
	private void sortInstances(Instances fulldata, double[][] iindex,
		...
		for (int x = 0; x < iindex[0].length; x++) {
			if (iindex[0][x] == 0 && iindex[1][x] == 0) // skip "discarded"
														// instances
				continue;
		...
		son(subset)
				.traverseTree(fulldata, iindex,
						test.minsAndMaxs(fulldata, limits, subset), this, pL,
						nodeClass);
	}
	 ...
	 * 
	 * @param fulldata
	 *            the instances in whole trainset
	 * @param iindex
	 *            records num tests each instance has failed up to this node
	 * @param limits
	 *            the upper/lower limits for numeric attributes
	 * @param parent
	 *            the node immediately before the current one
	 * @param pLaplace
	 *            laplace for leaf, calculated by parent (in case leaf empty)
	 * @param pLeafClass
	 *            class of leaf, determined by parent (in case leaf empty)
	 */
	private void findGraft(Instances fulldata, double[][] iindex,
		...
		int leafClass = (m_isEmpty) ? pLeafClass : localModel().distribution()
				.maxClass();

		// get the laplace value for this leaf
		double leafLaplace = (m_isEmpty) ? pLaplace : laplaceLeaf(leafClass);

		// sort the instances into those at the leaf, those in atbop, and
		// discarded
		...
		for (int cVal = 0; cVal < n.numClasses(); cVal++) {
			double theLaplace = (classDist[cVal] + 1.0)
					/ (classDist[cVal] + 2.0);
			if (cVal != leafClass
					&& (theLaplace > leafLaplace)
		...
		for (int a = 0; a < n.numAttributes(); a++) {
			...
			if (n.attribute(a).isNumeric()) {
				...
				// find minimal and maximal values for this attribute at the
				// leaf
				...
				for (int x = 0; x < n.numInstances(); x++) {
					...
					if (theval <= limits[a][0]) {
						if (m_Debug)
							System.out
									.println("\t  <= lowerlim: continuing...");
						continue;
					}
					// note: error in paper would have this read "theVal >
					// minLeaf)
					if (theval >= minLeaf) {
						if (m_Debug)
				...
				if (!Double.isNaN(minBestVal)
					...
					try {
						gsplit = new GraftSplit(a, minBestVal, 0, leafClass,
								minBestCounts);
					} catch (Exception e) {
						System.err.println("graftsplit error: "
								+ e.getMessage());
						System.exit(1);
					}
					t.add(gsplit);
				...
				if (n.numInstances() >= 1
					...
					for (int x = n.numInstances() - 1; x >= 0; x--) {
						...
						if (theval > limits[a][1]) {
							if (m_Debug)
								System.out
										.println("\t  >= upperlim; continuing...");
							continue;
						}
						if (theval <= maxLeaf) {
							if (m_Debug)
								System.out
										.println("\t  < maxLeaf; breaking...");
							break;
						}
						...
						if (x != 0 && !n.instance(sorted[x - 1]).isMissing(a)) {
							...
							while (z >= 0
								...
								z--;
								x--;
								counts[1][(int) n.instance(sorted[x])
										.classValue()] += iindex[1][sorted[x]];
							}
						}
				...
				for (int aval = 0; aval < n.attribute(a).numValues(); aval++) {
					for (int x = 0; x < l.numInstances(); x++) {
						if ((l.instance(x).isMissing(a) || l.instance(x).value(
								a) == aval)
		...
		for (int x = 0; x < t.size(); x++) {
			...
			if (gs.maxClassForSubsetOfInterest() != leafClass) {
				break; // reached a graft with class != leafClass, so stop
						// deleting
			} else {
				t.remove(x);
				x--;
		...
		for (int x = t.size() - 1; x >= 0; x--) {
			...
			try {
				...
				gs.deleteGraftedCases(l); // so they don't go down the other
											// branch
			} catch (Exception e) {
	...
	/**
	 * sorts the int array in ascending order by attribute indexed by a in
	 * dataset data.
	 * 
	 * @param the
	 *            data the indices represent
	 * @param the
	 *            index of the attribute to sort by
	 ...
	 *             if something goes wrong
	 */
	public double getProbsLaplace(int classIndex, Instance instance,
		...
		if (m_isLeaf) {
			...
			if (treeIndex == -1) {
				...
				for (i = 0; i < m_sons.length; i++) {
					if (!son(i).m_isEmpty) {
						if (!son(i).m_isLeaf) {
							prob += son(i).getProbsLaplace(classIndex,
									instance, weights[i] * weight);
						} else {
							prob += weight
									* weights[i]
	 ...
	 *             if something goes wrong
	 */
	public double getProbs(int classIndex, Instance instance, double weight)
			throws Exception {
	...
	/**
	 * add the grafted nodes at originalLeaf's position in tree. a recursive
	 * function that terminates when t is empty.
	 * 
	 * @param t
	 *            the list of nodes to graft
	 * @param originalLeaf
	 *            the leaf that the grafts are replacing
	 */
	public void setDescendents(ArrayList<GraftSplit> t,
		...
		if (t.size() > 0) {
			...
			newNode = new C45PruneableClassifierTreeG(m_toSelectModel,
					headerInfo, splitmod, m_pruneTheTree, m_CF,
					m_subtreeRaising, false, m_relabel, m_cleanup);
		} else {
			// get the leaf for one of newNode's children
			NoSplit kLeaf = ((GraftSplit) localModel()).getOtherLeaf();
			newNode = new C45PruneableClassifierTreeG(m_toSelectModel,
					headerInfo, kLeaf, m_pruneTheTree, m_CF, m_subtreeRaising,
		...
		if (m_sons != null) {
			...
			C45PruneableClassifierTreeG kNode = new C45PruneableClassifierTreeG(
					m_toSelectModel, headerInfo, kLeaf, m_pruneTheTree, m_CF,
					m_subtreeRaising, true, m_relabel, m_cleanup);
		...
		if (!end)
			((C45PruneableClassifierTreeG) newNode).setDescendents(t,
					(C45PruneableClassifierTreeG) originalLeaf);
	}
	 ...
	 * @return returns the probability of obtaining x or MORE out of n if r
	 *         proportion of n are positive.
	 * 
	 *         z for normal estimation of binomial probability of obtaining x or
	 *         more out of n, if r proportion of n are positive
	 */
		...
		try {
			...
			if (m_isLeaf) {
				...
				if (m_localModel instanceof GraftSplit)
					text.append(((GraftSplit) m_localModel).dumpLabelG(0,
							m_train));
				else
	...
	protected void dumpTree(int depth, StringBuffer text) throws Exception {
		...
		for (i = 0; i < m_sons.length; i++) {
			...
			if (m_sons[i].m_isLeaf) {
				text.append(": ");
				if (m_localModel instanceof GraftSplit)
					text.append(((GraftSplit) m_localModel).dumpLabelG(i,
							m_train));
				else
					text.append(m_localModel.dumpLabel(i, m_train));
			} else
				((C45PruneableClassifierTreeG) m_sons[i]).dumpTree(depth + 1,
						text);
		}
	}
