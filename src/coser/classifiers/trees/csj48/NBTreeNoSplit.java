/*
 *  Copied from NBTreeNoSplit.java
 *
 */
...
public final class NBTreeNoSplit extends ClassifierSplitModel {
	 ...
	 * 
	 * @param fullModel
	 *            a <code>NaiveBayesUpdateable</code> value
	 * @param trainingSet
	 *            an <code>Instances</code> value
	 * @param r
	...
	public static double crossValidate(NaiveBayesUpdateable fullModel,
			Instances trainingSet, Random r) throws Exception {
		...
		for (int j = 0; j < 5; j++) {
			...
			for (int k = 0; k < test.numInstances(); k++) {
				...
				((NaiveBayesUpdateable) copies[j]).updateClassifier(test
						.instance(k));
